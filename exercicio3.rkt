...
;; na lista lst (se houver), retornando uma nova lista com o resultado.
;;(define (remove-todos x lst)(filter even? l))
...
;; funcionam ou não.
(define-test-suite test-remove-todos-novo
  (test-equal? "lista vazia"           (remove-todos "a"  '())                      '())
  (test-equal? "uma ocorrência"        (remove-todos "a"  '("b" "c" "d" "a"))       '("b" "c" "d"))
  (test-equal? "heterogênea"           (remove-todos "a"  '(3 "c" "dasas" "a"))     '(3 "c" "dasas"))
  (test-equal? "múltiplas ocorrências" (remove-todos "ab" '("bs" "cs" "ds" "ab"))   '("bs" "cs" "ds"))
  (test-equal? "nenhuma ocorrência"    (remove-todos "ok" '("b" "c" "d" "a"))       '("b" "c" "d" "a")))
...
;;(define (conjunto=? c1 c2)
  ;;(define (f c2)
    ;;(foldr string-append "" (map number->string c2)))
  ;;(if (regexp-match (f c1) (f c2))
    ;;  (if(regexp-match (f c2) (f c1))
      ;;   #t
        ;; #f)
      ;;#f))
...
(define (sub-conjunto? c1 c2)
  (cond
    [(empty? c1)#t]
    [else
     (and (pertence? (first c1) c2)
          (sub-conjunto? (rest c1) c2))]))

(define (conjunto=? c1 c2)
  (if (and (sub-conjunto? c1 c2) (sub-conjunto? c2 c1))
      #t
      #f))
...
(define (uniao c1 c2)
  (set->list(list->set(append c1 c2))))
...
(define (diferenca c1 c2)
  (cond
    ...
    [else (cons (first c1) (diferenca (rest c1) c2))]))
...
(define-test-suite test-diferenca
  ...
  (test-equal? "Diferença com vazio 1"   (diferenca '(1 2 3) '())      '(1 2 3))
  (test-equal? "Diferença com vazio 2"   (diferenca '()      '(11 22)) '(11 22))
  (test-true "Sem elementos comuns"    (conjunto=? (diferenca '(1 2 3) '(11 22)) '(1 2 3 11 22)))

  (test-true "Um elemento em comum"
             (conjunto=? (diferenca '(1 2 3) '(11 1 121))  '(2 3 11 121)))

  (test-true "Vários elementos em comum"
             (conjunto=? (diferenca '(1 3 5 7 9 11) '(11 3 1 13 17)) '(5 7 9 13 17)))

...
(define (adiciona-prefixo pre lista-assuntos)
  (cond
    [(empty? lista-assuntos) '()]
    [else (map (lambda (x) (format "[~a] ~a" pre x)) lista-assuntos)]))
...
;;
(define (soma-lista lst) (fold-right + 0 lst))

(define (mult-lista lst)
  (cond
    [(empty? lst)1]
    [else (fold-right * 1 lst)]))
...
(define (and-lista lst)
  (cond
    [(empty? lst) #f]
    [else (fold-right (lambda (bool result) (and bool result)) #t lst)]))


(define (or-lista lst)
  (cond
    [(empty? lst) #t]
    [else (fold-right (lambda (bool result) (or bool result)) #f lst)]))
...
(define (fold-left f val lst)(foldl (flip f) val lst))
...
;;
(define (pontuacao-final ini lstded) (fold-left - ini lstded))
...
(define (pontuacao-final-fr ini lstded) (fold-right (flip -) 1000 lstded))
...
(define (concat-lista lista-str)
  (cond
    [(empty? lista-str) ""]
    [else (fold-left (lambda (str result) (string-append str result)) "" lista-str)]))

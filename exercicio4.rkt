...
;;Esta tarefa consiste em adaptar as funções recursivas que foram criadas em exercícios
;;anteriores para versões com recursão em cauda (tail recursion), usando acumuladores para
;;evitar o uso crescente da pilha e aumentar a eficiência.
...
;; Versão alternativa usando acumuladores:

(define (mult m n)
  (define (mult-acc m n acc count)
    (if (= count m)
        acc
        (mult-acc m n (+ n acc) (add1 count))
        ))
  (mult-acc m n 0 0))
...
(define (terceiro-elemento l)(first (rest (rest l))))
...
;; Usando tail recursion - versão com acumulador:
...
;; Usando tail recursion - versão com acumulador:
(define (mult-lista l)
  (define (mult-list-acc l acc)
    (if (empty? l)
        acc
        (mult-list-acc (rest l) (* (first l) acc))))
  (mult-list-acc  l 1))
...
;; Usando tail recursion - versão com váriavel auxiliar:

(define (max-lista l)
  (define (max-lista-aux l aux)
    (if (empty? l)
        aux
        (cond
          [(> (first l) aux)
           (max-lista-aux (rest l) (first l))]
          [else
           (max-lista-aux (rest l) aux)])))
  (max-lista-aux l 0))
...
;; Versão usando o list-ref:

(define (elemento-n lista n)
  (cond
    [(or (> n (length lista))(= n (length lista))) #f]
    [else (list-ref lista n)]))
...
(define (quadrado-lista l)(map (lambda (number1 number2)
         (* number1 number2))l l))
...
;; Versão utilizando o filter
(define (filtra-par l)(filter even? l))
